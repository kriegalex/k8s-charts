...
| persistence | object | `{"config":{"accessMode":"ReadWriteOnce","globalMounts":[{"path":"/config"}],"size":"2Gi","type":"persistentVolumeClaim"}}` | Persistent /config volume. To create a new PVC, set accessMode + size (+ storageClass if non-default). To reuse an existing PVC (e.g. Longhorn), set existingClaim instead. These two options are mutually exclusive per the bjw-s schema. |
